...
export const ReportFilters: React.FC<ReportFiltersProps> = ({
  ...
  const handleGranularityChange = (value: string) => {
    onFilterChange({
      ...filters,
      granularity: value as ReportFiltersType['granularity'],
    });
  };
  ...
  return (
    <Card className="p-6 border-l-4 border-l-primary bg-card shadow-card">
      <div className="space-y-6">
        ...
        {/* Granularity (Funnel) */}
        <div className="space-y-2">
          <Label htmlFor="granularity" className="text-sm font-medium text-foreground">Granularity</Label>
          <Select value={filters.granularity || 'date'} onValueChange={handleGranularityChange} disabled={isLoading}>
            <SelectTrigger id="granularity" className="bg-background border-input">
              <SelectValue placeholder="Select granularity" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="date">Datewise</SelectItem>
              <SelectItem value="month">Monthwise</SelectItem>
              <SelectItem value="year">Yearwise</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input id="region" value={filters.region || ''} onChange={(e) => onFilterChange({ ...filters, region: e.target.value })} disabled={isLoading} placeholder="e.g. North, South" />
        </div>
